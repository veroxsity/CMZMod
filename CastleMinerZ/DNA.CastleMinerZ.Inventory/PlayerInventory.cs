	...
	public class PlayerInventory
	{
		...
		public void DropOneSelectedTrayItem()
		{
			if (InventoryTray[SelectedInventoryIndex] != null)
			{
				...
				if (InventoryTray[SelectedInventoryIndex].StackCount == 1)
				{
					Vector3 localPosition = _player.LocalPosition;
					localPosition.Y += 1f;
					PickupManager.Instance.CreatePickup(InventoryTray[SelectedInventoryIndex], localPosition, true);
					InventoryTray[SelectedInventoryIndex] = null;
				}
				else
				{
					InventoryItem item = InventoryTray[SelectedInventoryIndex].PopOneItem();
		...
		public void GivePerks()
		{
			...
			if (CastleMinerZGame.Instance.FrontEnd.PromoCodes[9].Redeemed)
			{
				AddInventoryItem(InventoryItem.CreateItem(InventoryItemIDs.IronSpaceAssultRifle, 1));
			}
			if (CastleMinerZGame.Instance.FrontEnd.PromoCodes[10].Redeemed)
			{
				AddInventoryItem(InventoryItem.CreateItem(InventoryItemIDs.Grenade, 5));
			}
			if (CastleMinerZGame.Instance.FrontEnd.PromoCodes[11].Redeemed)
			{
				AddInventoryItem(InventoryItem.CreateItem(InventoryItemIDs.TNT, 5));
			}
			if (CastleMinerZGame.Instance.FrontEnd.PromoCodes[12].Redeemed)
			{
				AddInventoryItem(InventoryItem.CreateItem(InventoryItemIDs.RocketLauncher, 1));
			}
			if (CastleMinerZGame.Instance.FrontEnd.PromoCodes[13].Redeemed)
			{
				AddInventoryItem(InventoryItem.CreateItem(InventoryItemIDs.IronLaserSword, 1));
			}
		}
