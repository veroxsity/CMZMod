...
namespace DNA.CastleMinerZ.Inventory
{
	public class BoltRifleInventoryItemClass : GunInventoryItemClass
	{
		...
		public override Entity CreateEntity(ItemUse use, bool attachedToLocalPlayer)
		{
			...
			case ItemUse.UI:
			{
				Quaternion rotation = Quaternion.CreateFromYawPitchRoll(0f, 0f, -0.40840703f) * Quaternion.CreateFromYawPitchRoll(0f, -(float)Math.PI / 2f, 0f);
				Matrix localToParent2 = Matrix.Transform(Matrix.CreateScale(38.4f / modelEntity.GetLocalBoundingSphere().Radius), rotation);
				localToParent2.Translation = new Vector3(4f, -12f, -16f);
