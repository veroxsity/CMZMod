...
namespace DNA.CastleMinerZ.Utils.Threading
{
	public class TaskDispatcher
	{
		...
		private TaskDispatcher()
		{
			...
			{
				int num2 = _coreTable[j];
				TaskThread thread = _taskThreads[j];
				_systemThreads[j] = new Thread((ThreadStart)delegate
				{
					Thread.CurrentThread.SetProcessorAffinity(_coreTable[(int)thread._threadIndex]);
					thread.ThreadStart();
				});
			}
			if (_numThreads > 3)
			...
			for (int num = 0; num != _numThreads; num++)
			{
				_systemThreads[num].Name = _threadNames[num];
				_systemThreads[num].Start();
			}
		}
