...
namespace DNA.CastleMinerZ.Utils.Trace
{
	public class TraceProbe
	{
		...
		public virtual void Reset()
		{
			...
			_outT = 0f;
			_collides = false;
			ShapeHasSlopedSides = false;
			SimulateSlopedSides = false;
			FoundSlopedBlock = false;
			SlopedBlockT = 0f;
		}
		...
		public bool SetIntersection(float inT, ref Vector3 inNormal, bool startsIn, BlockFace inFace, float outT, ref Vector3 outNormal, bool endsIn, BlockFace outFace, IntVector3 worldindex)
		{
			...
			if (startsIn)
			{
				if (SimulateSlopedSides && ShapeHasSlopedSides)
				{
					FoundSlopedBlock = true;
					SlopedBlock = worldindex;
					SlopedBlockT = 0f;
					return result;
				}
				if (SkipEmbedded)
			...
			{
				if (SimulateSlopedSides && ShapeHasSlopedSides && inFace < BlockFace.POSY)
				{
					if (inT <= SlopedBlockT)
					{
						FoundSlopedBlock = true;
