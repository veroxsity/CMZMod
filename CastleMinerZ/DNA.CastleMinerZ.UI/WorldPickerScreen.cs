...
namespace DNA.CastleMinerZ.UI
{
	public class WorldPickerScreen : LongListScreen
	{
		...
		protected override void OnDraw(GraphicsDevice device, SpriteBatch spriteBatch, GameTime gameTime)
		{
			...
			Rectangle rectangle = new Rectangle(viewport.TitleSafeArea.Left, viewport.TitleSafeArea.Top + largeFont.LineSpacing * 2, viewport.TitleSafeArea.Width, viewport.TitleSafeArea.Height - largeFont.LineSpacing * 2);
			destRect = rectangle;
			spriteBatch.Begin();
		...
		{
			if ((controller.PressedButtons.X || inputManager.Keyboard.WasKeyPressed(Keys.X)) && base.SelectedItem is WorldPickerMenuItem)
			{
		...
		private void RenameWorld(WorldInfo world)
		{
			...
			Guide.BeginShowKeyboardInput(Screen.SelectedPlayerIndex.Value, "Rename " + world.Name, "Enter a New Name", world.Name, delegate(IAsyncResult result)
			{
