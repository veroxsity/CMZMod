...
namespace DNA.CastleMinerZ.UI
{
	public class InGameHUD : Screen
	{
		...
		private void DrawAcheivement(GraphicsDevice device, SpriteBatch spriteBatch)
		{
			...
			Vector2 vector4 = _game._systemFont.MeasureString(_achievementText2);
		...
		protected override void OnDraw(GraphicsDevice device, SpriteBatch spriteBatch, GameTime gameTime)
		{
			...
			if (!LocalPlayer.Dead && LocalPlayer.ShoulderedAnimState)
			{
				...
				if (gunInventoryItemClass != null && gunInventoryItemClass.Scoped)
				{
					...
					Vector2 position = new Vector2(titleSafeArea.Center.X - _sniperScope.Width / 2, titleSafeArea.Center.Y - _sniperScope.Height / 2);
					spriteBatch.Draw(_sniperScope, position, Color.White);
					spriteBatch.Draw(_game.DummyTexture, new Rectangle(0, 0, 1280, (int)position.Y), Color.Black);
					spriteBatch.Draw(_game.DummyTexture, new Rectangle(0, (int)position.Y + _sniperScope.Height, 1280, (int)position.Y), Color.Black);
					spriteBatch.Draw(_game.DummyTexture, new Rectangle(0, (int)position.Y, (int)position.X, _sniperScope.Height), Color.Black);
					spriteBatch.Draw(_game.DummyTexture, new Rectangle(1280 - (int)position.X, (int)position.Y, (int)position.X, _sniperScope.Height), Color.Black);
				}
				else
				{
			...
			else
			{
				...
				Vector2 position2 = new Vector2(titleSafeArea.Center.X, titleSafeArea.Center.Y);
				int num2 = 0;
				while (num2 < _damageIndicator.Count)
				{
					...
					_damageArrow.Draw(spriteBatch, position2, sourceRectangle, new Color((int)(139f * _damageIndicator[num2].fadeAmount), 0, 0, (int)(255f * _damageIndicator[num2].fadeAmount)), rotation, origin, 0.75f, SpriteEffects.None, 0f);
					num2++;
				}
				if (LocalPlayer.Dead && timeToShowRespawnText.Expired && _game.GameScreen._uiGroup.CurrentScreen == this)
				{
					...
					{
						if (_game.GameMode != GameModeTypes.Endurance || !AllPlayersDead() || _game.CurrentNetworkSession.IsHost)
						{
				...
				Vector2 position3 = new Vector2(destinationRectangle.Left, destinationRectangle.Top - 30);
				float num3 = PlayerHealth / 1f;
				_emptyHealthBar.Draw(spriteBatch, position3, Color.White);
				_fullHealthBar.Draw(spriteBatch, position3, new Rectangle(0, 0, (int)((float)_fullHealthBar.Width * num3), _fullHealthBar.Height), Color.White);
				...
				if (ActiveInventoryItem != null)
				{
					...
					spriteBatch.DrawOutlinedText(_game._medFont, sbuilder, new Vector2(destinationRectangle.Left, position3.Y - vector7.Y), Color.White, Color.Black, 2);
				}
		...
		protected override void OnUpdate(DNAGame game, GameTime gameTime)
		{
			...
			if (!fadeInGameStart.Expired && LocalPlayer.Avatar.AvatarState != AvatarRendererState.Loading)
			{
			...
			if (!gameBegun && _game.GameMode == GameModeTypes.Endurance && _game.IsOnlineGame)
			{
				if (_game.CurrentNetworkSession.IsHost)
				{
					...
					{
						_game.CurrentNetworkSession.SessionProperties[4] = 1;
						gameBegun = true;
						Console.WriteLine("The Game Has Begun - No Other Players Can Join");
					}
				}
				else if (_game.CurrentNetworkSession.SessionProperties[4] == 1)
				{
					gameBegun = true;
					Console.WriteLine("The Game Has Begun - No Other Players Can Join");
				}
		...
		public void Shoot(GunInventoryItemClass gun)
		{
			...
			{
				ShotgunShotMessage.Send((LocalNetworkGamer)LocalPlayer.Gamer, localToWorld, gun.Innaccuracy, gun.ID, gun.NeedsDropCompensation);
			}
			else
			{
				GunshotMessage.Send((LocalNetworkGamer)LocalPlayer.Gamer, localToWorld, gun.Innaccuracy, gun.ID, gun.NeedsDropCompensation);
			}
		}
		...
		public override void OnLostFocus()
		{
			...
			OnPlayerInput(new InputManager(_game), new GameController(Screen.SelectedPlayerIndex.Value), new KeyboardInput(Screen.SelectedPlayerIndex.Value), gameTime);
			base.OnLostFocus();
		}
		...
		protected override bool OnPlayerInput(InputManager inputManager, GameController controller, KeyboardInput chatPad, GameTime gameTime)
		{
			...
			GunEyePointCameraLocation += (vector2 - GunEyePointCameraLocation) * num * (float)gameTime.ElapsedGameTime.TotalSeconds;
			...
			else
			{
				...
				else if ((inputManager.Keyboard.WasKeyPressed(Keys.D1) || inputManager.Keyboard.WasKeyPressed(Keys.NumPad1)) && !LocalPlayer.UsingAnimationPlaying)
				{
					SoundManager.Instance.PlayInstance("Click");
				...
				else if ((inputManager.Keyboard.WasKeyPressed(Keys.D2) || inputManager.Keyboard.WasKeyPressed(Keys.NumPad2)) && !LocalPlayer.UsingAnimationPlaying)
				{
					SoundManager.Instance.PlayInstance("Click");
				...
				else if ((inputManager.Keyboard.WasKeyPressed(Keys.D3) || inputManager.Keyboard.WasKeyPressed(Keys.NumPad3)) && !LocalPlayer.UsingAnimationPlaying)
				{
					SoundManager.Instance.PlayInstance("Click");
				...
				else if ((inputManager.Keyboard.WasKeyPressed(Keys.D4) || inputManager.Keyboard.WasKeyPressed(Keys.NumPad4)) && !LocalPlayer.UsingAnimationPlaying)
				{
					SoundManager.Instance.PlayInstance("Click");
				...
				else if ((inputManager.Keyboard.WasKeyPressed(Keys.D5) || inputManager.Keyboard.WasKeyPressed(Keys.NumPad5)) && !LocalPlayer.UsingAnimationPlaying)
				{
					SoundManager.Instance.PlayInstance("Click");
				...
				else if ((inputManager.Keyboard.WasKeyPressed(Keys.D6) || inputManager.Keyboard.WasKeyPressed(Keys.NumPad6)) && !LocalPlayer.UsingAnimationPlaying)
				{
					SoundManager.Instance.PlayInstance("Click");
				...
				else if ((inputManager.Keyboard.WasKeyPressed(Keys.D7) || inputManager.Keyboard.WasKeyPressed(Keys.NumPad7)) && !LocalPlayer.UsingAnimationPlaying)
				{
					SoundManager.Instance.PlayInstance("Click");
				...
				else if ((inputManager.Keyboard.WasKeyPressed(Keys.D8) || inputManager.Keyboard.WasKeyPressed(Keys.NumPad8)) && !LocalPlayer.UsingAnimationPlaying)
				{
					SoundManager.Instance.PlayInstance("Click");
				...
				if (inputManager.Keyboard.WasKeyPressed(Keys.Q))
				{
