	...
	public class EnemyManager : Entity, IGameMessageHandler
	{
		...
		private void CheckDragonBox(GameTime time)
		{
			...
			else if (time.TotalGameTime > NextSpawnDragonTime)
			{
				...
				Vector3 worldPosition = localPlayer.WorldPosition;
				worldPosition.Y = 0f;
				if (NextTimedDragonType < DragonTypeEnum.COUNT || worldPosition.LengthSquared() > 12960000f)
				{
		...
		public void HandleDetonateFireballMessage(DetonateFireballMessage msg)
		{
			...
			for (byte b = 0; b < msg.NumBlocks; b++)
			{
		...
		public void HandleEnemyGiveUpMessage(EnemyGiveUpMessage msg)
		{
			for (int i = 0; i < _enemies.Count; i++)
			{
				if (!_enemies[i].IsDead && _enemies[i].Target.Gamer.Id == msg.TargetID && _enemies[i].EnemyID == msg.EnemyID)
				{
					_enemies[i].GiveUp();
					break;
				}
		...
		public void HandleKillEnemyMessage(KillEnemyMessage msg)
		{
			for (int i = 0; i < _enemies.Count; i++)
			{
				...
				if (CastleMinerZGame.Instance.IsLocalPlayerId(msg.KillerID))
				{
					...
					_enemies[i].CreatePickup();
					if (_enemies[i].EType.FoundIn == EnemyType.FoundInEnum.ABOVEGROUND)
					{
		...
		public void HandleSpeedUpEnemyMessage(SpeedUpEnemyMessage msg)
		{
			for (int i = 0; i < _enemies.Count; i++)
			{
				if (_enemies[i].Target.Gamer.Id == msg.TargetID && _enemies[i].EnemyID == msg.EnemyID)
				{
					if (!_enemies[i].IsMovingFast && !_enemies[i].IsDead)
					{
						_enemies[i].SpeedUp();
					}
					break;
		...
		private void SpawnAlien(Vector3 plrpos, bool inAsteroid, float gametime)
		{
			...
			{
				zero = BlockTerrain.Instance.FindNearbySpawnPoint(plrpos, spawnRadius * 2, spawnRadius);
			}
			else
			{
				int num;
				...
				num3 = ((num3 > 0) ? (num3 + num2) : (num3 - num2));
				zero.Z += num3;
				zero = ((!BlockTerrain.Instance.RegionIsLoaded(zero)) ? Vector3.Zero : BlockTerrain.Instance.FindAlienSpawnPoint(zero, _closestSpaceRock > (float)spawnRadius));
			}
			if (zero.LengthSquared() != 0f)
			{
				Vector3 pos = new Vector3(zero.X, zero.Y + 0.5f, zero.Z);
				IntVector3 a = new IntVector3((int)zero.X, (int)zero.Y - 1, (int)zero.Z);
				int num4 = BlockTerrain.Instance.MakeIndexFromWorldIndexVector(a);
				...
				if (type.ParentBlockType == BlockTypeEnum.SpaceRock || _closestSpaceRock > (float)(spawnRadius * 2))
				{
					...
					SpawnEnemyMessage.Send((LocalNetworkGamer)CastleMinerZGame.Instance.LocalPlayer.Gamer, pos, enemyTypeEnum, 0f, MakeNextEnemyID(), _rnd.Next());
					_localAlienCount++;
				}
			}
		}
		...
		{
			BaseZombie z = new BaseZombie(this, EnemyTypeEnum.ALIEN, CastleMinerZGame.Instance.LocalPlayer, plrpos, 51, 1, EnemyType.Types[51].CreateInitPackage(0.5f));
			AddZombie(z);
		}
		...
		public void SearchForSpaceRock(TaskThreadEnum threadIndex, BaseTask task, object context)
		{
			...
			if (instance != null)
			{
				flag = instance.ContainsBlockType(_spaceRockScanPosition, EnemyType.Types[51].SpawnRadius * 3, BlockTypeEnum.SpaceRock, ref distToClosest);
				if (flag)
				{
