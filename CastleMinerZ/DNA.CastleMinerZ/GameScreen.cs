...
namespace DNA.CastleMinerZ
{
	public class GameScreen : ScreenGroup
	{
		...
		private RenderTarget2D _envMap;
		...
		{
			for (int i = 0; i < _explosiveFlashEntities.Count; i++)
			{
				if (_explosiveFlashEntities[i].BlockPosition == position)
				{
					_explosiveFlashEntities[i].RemoveFromParent();
					_explosiveFlashEntities.RemoveAt(i);
					break;
				}
			}
		}
		...
		public void Inialize()
		{
			...
			_envMap = new RenderTarget2D(_game.GraphicsDevice, presentationParameters.BackBufferWidth / 4, presentationParameters.BackBufferHeight / 4, true, presentationParameters.BackBufferFormat, presentationParameters.DepthStencilFormat, 1, RenderTargetUsage.DiscardContents);
			...
			CameraView item = new CameraView(_envMap, _localPlayer.FPSCamera, FilterWorldGeo);
			...
			CameraView item2 = new CameraView(null, _localPlayer.GunEyePointCamera);
			sceneScreen.Views.Add(item2);
		}
		...
		private void _hostOptionsMenu_MenuItemSelected(object sender, SelectedMenuItemArgs e)
		{
			...
			case HostOptionItems.PVP:
			{
				...
				case CastleMinerZGame.PVPEnum.Everyone:
					message = "PVP: Everyone";
					break;
				case CastleMinerZGame.PVPEnum.NotFriends:
					message = "PVP: Non-Friends Only";
					break;
