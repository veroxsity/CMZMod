...
namespace DNA.CastleMinerZ
{
	public class Selector : ModelEntity
	{
		...
		{
			u1 += (float)gameTime.ElapsedGameTime.TotalSeconds / 5f;
			u2 -= (float)gameTime.ElapsedGameTime.TotalSeconds / 5f;
			u1 -= (int)u1;
			u2 -= (int)u2;
		...
		protected override bool SetEffectParams(ModelMesh mesh, Effect effect, GameTime gameTime, Matrix world, Matrix view, Matrix projection)
		{
			...
			if (mesh.Name == "Box001")
			{
				effect.Parameters["uvOffset"].SetValue(new Vector2(u1, 0f));
			}
			if (mesh.Name == "Box002")
			{
				effect.Parameters["uvOffset"].SetValue(new Vector2(u2, 0f));
			}
