...
namespace DNA.CastleMinerZ
{
	public class CastleMinerZPlayerStats : PlayerStats
	{
		...
		protected override void SaveData(BinaryWriter writer)
		{
			...
			writer.Write(AlienEncounters);
			writer.Write(DragonsKilledWithGuidedMissile);
			writer.Write(EnemiesKilledWithTNT);
			writer.Write(EnemiesKilledWithGrenade);
			writer.Write(EnemiesKilledWithLaserWeapon);
		...
		protected override void LoadData(BinaryReader reader, int version)
		{
			...
			if (version >= 4)
			{
				AlienEncounters = reader.ReadInt32();
				DragonsKilledWithGuidedMissile = reader.ReadInt32();
				EnemiesKilledWithTNT = reader.ReadInt32();
				EnemiesKilledWithGrenade = reader.ReadInt32();
				EnemiesKilledWithLaserWeapon = reader.ReadInt32();
			}
			if (version >= 5)
			{
