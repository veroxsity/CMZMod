...
namespace DNA.Drawing.Curves.Splines
{
	public class CubicBezierCurve : Spline
	{
		...
		private static Vector3 ComputeValue(float t, ControlPoint cp1, ControlPoint cp2)
		{
			...
			Vector3 vector = cp1.Out;
			Vector3 vector2 = cp2.In;
			...
			float num5 = t * t * t;
			float x = num2 * location.X + num3 * vector.X + num4 * vector2.X + num5 * location2.X;
			float y = num2 * location.Y + num3 * vector.Y + num4 * vector2.Y + num5 * location2.Y;
			float z = num2 * location.Z + num3 * vector.Z + num4 * vector2.Z + num5 * location2.Z;
