...
namespace DNA.Drawing
{
	public class Bone
	{
		...
		public Vector3 Scale
		{
			...
			set
			{
				if (_dirty || _scale != value)
				{
					_scale = value;
					_unitScale = _scale == Vector3.One;
					_dirty = true;
				}
			}
		}
		...
		private void ComposeTransform()
		{
			if (_unitScale)
			{
				Matrix.CreateFromQuaternion(ref _rotation, out _transform);
			}
			else
			{
				Matrix.CreateScale(ref _scale, out _transform);
				_transform *= Matrix.CreateFromQuaternion(_rotation);
			}
		...
		{
			return BuildSkeleton(avatarRenderer.BindPose, avatarRenderer.ParentBones, Avatar.BoneNames);
		}
