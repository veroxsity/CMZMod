...
namespace DNA.Drawing.UI
{
	public class BarSettingItem : SettingItemElement
	{
		...
		{
			if (_location.Contains(mouse.Position) && mouse.LeftButtonDown)
			{
				float num = mouse.Position.X - _location.X;
				Value = num / (float)_location.Width;
				return true;
			}
			return false;
		}
		...
		public override void OnDraw(DNAGame _game, GraphicsDevice device, SpriteBatch spriteBatch, SpriteFont font, Color textColor, Color outlineColor, int outlineWidth, float yLoc)
		{
			...
			_location = new Rectangle((int)((float)(titleSafeArea.Center.X + 60) + vector.X), (int)num3, (int)num, (int)num2);
