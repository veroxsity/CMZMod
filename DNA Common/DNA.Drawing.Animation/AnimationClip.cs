	...
	public class AnimationClip
	{
		...
		public void ReduceKeys()
		{
			for (int i = 0; i < BoneCount; i++)
			{
				...
				for (int l = 0; l < _scales[i].Length; l++)
				{
					if ((double)Math.Abs(_scales[i][l].X - 1f) < 0.001)
					{
						_scales[i][l].X = 1f;
					}
					if ((double)Math.Abs(_scales[i][l].Y - 1f) < 0.001)
					{
						_scales[i][l].Y = 1f;
					}
					if ((double)Math.Abs(_scales[i][l].Z - 1f) < 0.001)
					{
						_scales[i][l].Z = 1f;
					}
				}
				flag = true;
				for (int m = 1; m < _rotations[i].Length; m++)
				{
					if (_rotations[i][0] != _rotations[i][m])
					{
						flag = false;
						break;
					}
