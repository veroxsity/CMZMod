...
namespace DNA.Audio.SignalProcessing.Processors
{
	public class MicInputProcessor : BlockingSignalProcessor<RawPCMData>, IDisposable
	{
		...
		{
			_microphone = mic;
			_microphone.BufferDuration = TimeSpan.FromMilliseconds(100.0);
			handler = _microphone_BufferReady;
			_microphone.BufferReady += handler;
			_micBufferSize = _microphone.GetSampleSizeInBytes(_microphone.BufferDuration);
